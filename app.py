...
def calculate_breakouts(
    df, ticker, volume_breakout_threshold, price_change_threshold, holding_period
):
    ...
    df["Volume_Breakout"] = (
        df["Volume"] > (volume_breakout_threshold / 100) * df["Avg_Volume_20"]
    )
...
def analyze():
    ...
    if (
        ...
        or not price_change_threshold
        or not holding_period
    ):
    ...
    breakout_results = calculate_breakouts(
        df, ticker, volume_threshold, price_change_threshold, holding_period
    )
